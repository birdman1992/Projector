...
int getMaxFds()
{
    ...
    list_for_each(pos,&sktList.list){
        p=list_entry(pos,struct socketList,list);
        ret = (p->socketFd > ret)?p->socketFd:ret;
    }
...
//        printf("maxFd:%d\n",maxFd);
        ...
        if(select(maxFd+1, &fdsHost, NULL, NULL, &tv) > 0){
            for(i=0; i<=maxFd; i++)
            {
                printf("%d\n",i);
                if(FD_ISSET(i, &fdsHost)){
                    memset(buffer, 0, sizeof(buffer));
                    int recvSize = read(i, buffer, sizeof(buffer));
                    if(recvSize == 0)
                    {
                        socketClear(i);
                        FD_CLR(i,&fdsHost);
